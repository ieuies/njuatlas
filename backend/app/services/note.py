    ...
    def search(self, *, type=None, tags=None, place_id=None,
        ...
        if keyword:
            ...
            if term:
                ...
                user_ids = db.session.query(User.id).filter(User.username.ilike(like))
                q = q.filter(or_(
                    EventPost.title.ilike(like),
                    EventPost.content.ilike(like),
                    EventPost.location_name.ilike(like),
                    EventPost.budget.ilike(like),
                    EventPost.id.in_(tag_post_ids),
                    EventPost.user_id.in_(user_ids),
                ))
        ...
        if sort == "nearby":
            ...
            max_candidates = current_app.config.get("NEARBY_SORT_MAX_CANDIDATES", 400)
            all_rows = q.order_by(EventPost.hot_score.desc()).limit(max_candidates).all()
